...
(function(L, Terraformer){
  L.esri.ClusteredFeatureLayer = L.Class.extend({
    ...
    _setObjectIdField: function(response){
      if(response.objectIdFieldName){
        ...
        for (var j = 0; j <= response.fields.length - 1; j++) {
          if(response.fields[j].type === "esriFieldTypeOID" || response.fields[j].name === 'PRISM.DBO.SV_DMPROJECT1.OBJECTID') {
    ...
    _render: function(response){
      if(response.features && response.features.length > 0 && !response.error){
